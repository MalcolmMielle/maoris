...
class ZoneAsso {
   ...
   public:
    ...
    void sort() {
        std::sort(other_zones.begin(), other_zones.end(),
                  [](const ZoneTest& a, const ZoneTest& b) {
   ...
   public:
    ...
    void sort() {
        ...
        std::sort(zones.begin(), zones.end(),
                  [](const ZoneAsso& a, const ZoneAsso& b) {
...
inline void exportResultsGnuplot(const std::string& file_out,
                                 const std::string& file_studied,
                                 const AASS::maoris::results& Regions,
                                 double max,
                                 float proper_size) {
    ...
    if (myfile.is_open()) {
        myfile << name << " " << Regions.precision << " " << Regions.recall
               << " " << Regions.inverse_recall << " " << Regions.time << " "
               << max << " " << proper_size << "\n";
        myfile.close();
    } else
...
void compare_images(cv::Mat GT_segmentation_in,
                    cv::Mat DuDe_segmentation_in,
                    results& res_out);

void extract_results(results& Regions,
                     std::vector<float>& Precisions,
                     std::vector<float>& Recalls,
                     std::vector<float>& inverse_recall);
...
class Evaluation {
   protected:
    ...
    double _gscore;
    double _accuracy;
    ///@brief Matthew's correlation coefficient for the segmented map,
    /// calculated from the tp, fp, tn, fn. Not good because not normalized. DO
    /// NOT USE
    //			double _matthewCC;
    //			double _matthewCC_median;
    ///@brief Matthew's correlation coefficient for the segmented map,
    /// calculated for each zone individually.
    double _matthewCC_perzone;
    ///@brief median Matthew's correlation coefficient for the segmented map,
    /// calculated for each zone individually.
   ...
   public:
    ...
    /**
     * @brief Calculate all measures after all the segmentation have been
     * compared.
     */
    void calculate();
    ...
    /**
     * @brief compare the segmentation input with a ground truth. Call this
     * function for each segmented map in your dataset. Once it is done, use
     * calculate().
     * @param seg The Mat that was segmented. Each zone with a value.
     * @param GT_segmentation The ground truth segmentation Mat. It needs to be
     * the same size as the input. Each zone with a value
     * @param time The time taken to process the maps. Default is set to 0.
     * @param file Path to the file of the map that was processed. Default to
     * "name.png"
     */
    void compare(const cv::Mat& seg,
    ...
    std::vector<double> getAllMatthewsCCPerZone() {
        return _matthewCC_perzone_individual;
    };
   ...
   private:
    ...
    void compareImagesUnbiased(cv::Mat GT_segmentation_in,
                               cv::Mat DuDe_segmentation_in);
};
