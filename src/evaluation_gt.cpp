...
void process(const std::string& file, bool write = false) {
    ...
    // 	cv::resize(graphmat, graphmat, cv::Size(graphmat.cols * 4, graphmat.rows
    // * 4));
    cv::imshow("GRAPH Visible", graphmat);
    // //
    // // 	std::cout << "Zones " << graph_slam.getNumVertices() <<
    // std::endl;
    //
    //
    ...
    // * 2)); 	cv::imshow("GRAPH", img_hist_equalized); 	cv::waitKey(0);
    //
