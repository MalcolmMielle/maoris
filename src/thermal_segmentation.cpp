...
void draw(AASS::RSI::GraphZone& gp_real,
          AASS::RSI::GraphZone& gp_model,
          const cv::Mat& obstacle,
          const cv::Mat& obstacle_model,
          std::vector<std::pair<AASS::RSI::GraphZone::Vertex,
                                AASS::RSI::GraphZone::Vertex> > matches) {
    cv::Mat obst_copy;
...
void makeGraph(const std::string& file, AASS::RSI::GraphZone& graph_slam) {
    ...
    cv::Mat element = getStructuringElement(
        cv::MORPH_RECT, cv::Size(2 * 5 + 1, 2 * 5 + 1), cv::Point(5, 5));
