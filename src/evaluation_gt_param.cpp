...
void process(const std::string& file,
    ...
    // * 2)); cv::imshow("GRAPH", img_hist_equalized);
    //
    ...
    for (; test_what < which_to_end; ++test_what) {
        ...
        try {
            for (auto gt : gts) {
                ...
                if (!boost::filesystem::exists(p) ||
                    !boost::filesystem::exists(p_gt)) {
                    std::cout << "need a valid path toward the images"
                              << std::endl;
                    return 0;
                }
                ...
                if (boost::filesystem::is_directory(p)) {
                    for (t; t <= end; t = t + step) {
                        ...
                        // 			int i = 0;
                        for (std::vector<boost::filesystem::path>::
                            ...
                            std::cout << "Process " << fn.string()
                                      << " with model " << model << std::endl;
